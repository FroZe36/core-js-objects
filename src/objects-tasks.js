...
function makeWord(lettersObject) {
  let resultString = '';
  const valuesOfLetterPosition = Object.values(lettersObject).flat();
  valuesOfLetterPosition
    .sort((a, b) => a - b)
    .forEach((position) => {
      Object.entries(lettersObject).forEach(([letter, letterIndex]) => {
        if (letterIndex.includes(position)) {
          resultString += letter;
        }
      });
    });
...
const cssSelectorBuilder = {
  ...
  checkPosition(position) {
    if (
      [this.counts.id, this.counts.element, this.counts.pseudoElement].includes(
        position
      ) &&
